...
layout(std140) uniform QuadUniforms {
    vec4 uCoords;
    vec4 uCorners; // x: TR, y: BR, z: TL, w: BL
    vec4 uColor;
    float uBorderSize;
    vec4 uBorderColor;
    float splitBorder;
...
void main() {
    ...
    vec2 cornerChoice = (centerOffset.x > 0.0) ? uCorners.xy : uCorners.zw;
    float currentCornerRadius = (centerOffset.y > 0.0) ? cornerChoice.x : cornerChoice.y;


    vec2 qZone = abs(centerOffset) - halfSize;


    float inCornerZone = step(0.0, qZone.x) * step(0.0, qZone.y);


    float borderBypass = 1.0 - (step(currentCornerRadius, 0.0) * inCornerZone);
    float borderMask = baseBorderMask * borderBypass;
    ...
    if (splitBorder == 1.0) {
        ...
        finalBackgroundAlpha = mix(0.2, 0.0, normalizedX);
